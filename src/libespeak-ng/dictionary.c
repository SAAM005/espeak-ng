...
static void InitGroups(Translator *tr)
{
	...
	if (*p != RULE_GROUP_END) while (*p != 0) {
		...
		if (p[0] == RULE_REPLACEMENTS) {
			...
			p++;
			printf("RULE_REPLACEMENTS2:%s\n",printAround(p,20));
			continue;
		}

		if (p[0] == RULE_PREPLACEMENTS) {
			printf("RULE_PREPLACEMENTS1:%s\n",printAround(p,20));
			p = (char *)(((intptr_t)p+4) & ~3); // advance to next word boundary

			tr->langopts.preplace_chars = (unsigned char *)p;
